...
cat wiki_recentchange.json | sed -n "s/^data: //p" | jq 'select(.meta.domain == "en.wikipedia.org")' | jq ".meta.uri" | sort > recentchange_uris
echo "Recent_Change URIs $(cat recentchange_uris | wc -l)"
cat wiki_revision_score.json | sed -n "s/^data: //p" | jq 'select(.meta.domain == "en.wikipedia.org")' | jq ".meta.uri" | sort > revision_score_uris
echo "Revision_Score URIs $(cat revision_score_uris | wc -l)"
cat wiki_revision_create.json | sed -n "s/^data: //p" | jq 'select(.meta.domain == "en.wikipedia.org")' | jq ".meta.uri" | sort > revision_create_uris
