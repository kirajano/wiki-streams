...
The schema definition should follow the ["null", "type"] pattern. It is about transforming with jq in the right format (https://stackoverflow.com/questions/27485580/how-to-fix-expected-start-union-got-value-number-int-when-converting-json-to-av). The below successfully transforms the above mentioned fields(id, registraion date and edit count). Now, it needs to be tested about with the producer into a topic. The current problem lies in the producer part that there are errors. Very likely the schema in the command below neeeds to be looked at again.

```
kafka-console-consumer --topic wiki_revisionscore_raw --bootstrap-server broker:29092 | jq  '.performer.user_id |= if type == "number" then {"int": .} else null end | .performer.user_edit_count |= if type == "number" then {"int": .} else null end | .performer.user_registration_dt |= if type == "string" then {"string": .} else null end | {user_id: .performer.user_id, user_registration_dt: .performer.user_registration_dt, user_edit_count: .performer.user_edit_count}' | kafka-avro-console-producer --bootstrap-server broker:29092 --property schema.registry.url=http://schema-registry:8081 --topic wiki_revisionscore_testing --property value.schema='{"name": "revisionscore", "namespace": "mediawiki", "type": "record", "fields": [{"name": "user_id", "type": ["null", "int"]}, {"default": null, "name": "user_registration_dt", "type":["null", {"type": "string", "logicalType":"timestamp-millis"}]}, {"name": "user_edit_count", "type": ["null", "int"]}]}'
```
